...
const POST_IMAGE_WATERMARK_PATH = path.join(__dirname, 'images', 'theme', 'c93ec7d3fdd247f39295918728c1798b.png');
const POST_IMAGE_WATERMARK_OPACITY = 0.7;

/** Rotate, resize, composite watermark bottom-center (if file exists), write WebP */
async function writePostImageWebpWithWatermark(buf, outPath, resizeOpts, webpOpts = { quality: 85 }) {
  const base = sharp(buf).rotate().resize(resizeOpts);
  if (!fs.existsSync(POST_IMAGE_WATERMARK_PATH)) {
    await base.webp(webpOpts).toFile(outPath);
    return;
  }
  const meta = await base.clone().metadata();
  const imgW = meta.width || 800;
  const imgH = meta.height || 800;
  const maxOverlayW = Math.min(Math.max(48, Math.round(imgW * 0.22)), imgW);
  let wmSharp = sharp(POST_IMAGE_WATERMARK_PATH)
    .resize({
      width: maxOverlayW,
      height: imgH,
      fit: 'inside',
    ...
    .ensureAlpha();
  const { data: wmPx, info: wmInfo } = await wmSharp.raw().toBuffer({ resolveWithObject: true });
  const ch = wmInfo.channels || 4;
  if (ch >= 4) {
    const a = POST_IMAGE_WATERMARK_OPACITY;
    for (let i = ch - 1; i < wmPx.length; i += ch) {
      wmPx[i] = Math.min(255, Math.round(wmPx[i] * a));
    }
  }
  const wmBuffer = await sharp(wmPx, {
    raw: { width: wmInfo.width, height: wmInfo.height, channels: ch }
  })
    .png()
    .toBuffer();
  await base
    .composite([{ input: wmBuffer, gravity: 'south', blend: 'over' }])
    .webp(webpOpts)
    .toFile(outPath);
}
...
app.post('/api/dashboard/posts/upload', requireAuth, postUpload.single('image'), async (req, res) => {
  ...
  try {
    ...
    await writePostImageWebpWithWatermark(buf, outPath, { width: 1920, height: 2560, fit: 'inside', withoutEnlargement: true });
    await writePostImageWebpWithWatermark(buf, thumbPath, { width: 600, fit: 'inside', withoutEnlargement: true });
...
app.post('/api/dashboard/posts', requireAuth, postUpload.single('image'), async (req, res) => {
  ...
  // Match /api/dashboard/posts & /api/projects sort: lower Order first. (Note: Order 0 is falsy → treated as 999999 in sorts.)
  let newOrder = 1;
  if (posts.length) {
    const minKey = Math.min(...posts.map(p => (p.Order || 999999)));
    newOrder = minKey - 1;
...
app.post('/api/dashboard/posts/:id/gallery/upload', requireAuth, postUpload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const buf = req.file.buffer;
    await writePostImageWebpWithWatermark(buf, outPath, { width: 1920, height: 2560, fit: 'inside', withoutEnlargement: true });
    await writePostImageWebpWithWatermark(buf, thumbPath, { width: 300, fit: 'inside', withoutEnlargement: true });
    const relPath = path.relative(__dirname, outPath).replace(/\\/g, '/');
    const galleries = [...(db.Galleries || [])];
...
app.post('/api/dashboard/posts/:id/gallery/video', requireAuth, postUpload.fields([{ name: 'video', maxCount: 1 }, { name: 'image', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    await writePostImageWebpWithWatermark(imageFile.buffer, thumbOutPath, { width: 1920, height: 2560, fit: 'inside', withoutEnlargement: true });
    await writePostImageWebpWithWatermark(imageFile.buffer, thumbSmallPath, { width: 300, fit: 'inside', withoutEnlargement: true });
